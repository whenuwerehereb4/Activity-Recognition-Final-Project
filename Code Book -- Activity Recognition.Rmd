---
title: "Code Book — Activity Recognition"
author: "Andrew Nix"
date: "3/7/2017"
...
IMPORTANT NOTES TO KEEP IN MIND WHEN REVIEWING THE CODE BOOK: *Note 1:
The values listed in the code book come from 2 separate data frame
objects that are constructed in the attached R script and are the basis
for the analysis (details for how these objects are created are included
in the R script and README file): **Note 2:  The variable names are
identical in both objects.  In one case they refer to the original
observations and in the second case they represents a summation of those
observations.   For further clarification, here is an explanation of the
2 objects:
...
columns, subject and activity) in the
        subjxactivityxfeature_COMBINED (described above) data frame
across all observations associated with the the same subject AND
activity in the data - this is the source of the data output when the
attached R script is run


IDENTIFIER INFORMATION — refers to the grouping variables described
above 
——————————————————————————————————————————————————————— 

subject -
Unique identifier for the subject - the variable range is 1 to 30 (the
study includes 30 subjects)

——————————————————————————————————————————————————————— 
activity -
Qualitative identifier for the activity that was labeled by human
observers based on video reviews - The study involves measurements of 6
different activities. The activities that were recorded (and the
corresponding labels in the data) are as follows: 1)  WALKING 2) 
WALKING_UPSTAIRS 3)  WALKING_DOWNSTAIRS 4)   STANDING 5)  SITTING 6) 
LAYING 

———————————————————————————————————————————————————————

Overview of the Quantiative Data All data and details about the
measurements was gathered from here: 
...
following methodology: “… the accelerometer and gyroscope 3-axial raw
...
low pass Butterworth filter with a corner frequency of 0.3 Hz.” (taken
from the “features_info.txt” file within the ZIP file)

These variables have been grouped below according to the movement and
statistic type.

In addition: - The X,Y and Z associated with each of the groups refer to
whether the movement was made along the X,Y or Z axis. This is the same
for all movement types and is therefore not repeated below. - there are
2 statistic types associated with these groups: 1) mean ; and 2)
standard deviations - these statistics were calculated based on repeated
...
tbodyAcc-mean()-X tbodyAcc-mean()-Y tBodyAcc-mean()-Z - the body
component of the  acceleration signal, which has gravitational and body
motion components, was separated using a Butterworth low-pass filter
into body acceleration and gravity.

tBodyAcc-std()-X tBodyAcc-std()-Y tBodyAcc-std()-Z — corresponding
standard deviations
———————————————————————————————————————————————————————
tGravityAcc-mean()-X tGravityAcc-mean()-Y tGravityAcc-mean()-Z - the
gravity component of the sensor acceleration signal (which has
...
The following variables were determined by the following methodology:
...
*NOTE: the [ ] and “ “ are not par of the variable name
———————————————————————————————————————————————————————
